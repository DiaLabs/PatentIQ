...
const ReportPage = ({ children }: ReportPageProps) => (
  <div className="relative w-full max-w-[850px] min-h-0 sm:min-h-[1122px] bg-white dark:bg-zinc-900 p-5 sm:p-[15mm] md:p-[20mm] shadow-2xl mb-6 sm:mb-10 border border-gray-100 dark:border-zinc-800 overflow-hidden mx-auto first:mt-0 sm:first:mt-2 last:mb-4 group select-none transition-all rounded-sm sm:rounded-none">
...
export function ReportContent({ data }: ReportContentProps) {
  ...
  return (
    <div className="w-full font-sans text-gray-900 dark:text-gray-100 py-0 sm:py-4 px-0 sm:px-4 flex flex-col items-center">
      ...
      <ReportPage>
        {/* Header Section */}
        <div className="text-center mb-10 sm:mb-16 pt-4 sm:pt-8">
          <h1 className="text-xl sm:text-2xl md:text-3xl font-black tracking-tight mb-3 uppercase border-b-4 border-indigo-600 inline-block pb-1 text-gray-900 dark:text-white leading-tight">
            {data.document_title || "Patent Scoring and Improvement Report"}
          </h1>
          <h2 className="text-lg sm:text-xl font-bold text-gray-600 dark:text-gray-400 mt-4 max-w-2xl mx-auto leading-tight">
            {data.invention_title || "Invention Disclosure"}
          </h2>
          <p className="text-[10px] sm:text-sm font-medium text-gray-400 mt-4 uppercase tracking-[0.2em]">
            Internal Patent-Screening Evaluation
          </p>
        </div>

        {/* Meta Stats Table */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-10 mb-10 sm:mb-16">
                      ...
                      Object.entries(data.analysis || {}).map(([key, val]: any, i: number) => {
                        ...
                        return (
                          <tr key={i} className="hover:bg-gray-50/50 dark:hover:bg-zinc-800/30 transition-colors">
                            ...
                            <td className="px-6 py-5 text-[12px] text-gray-500 dark:text-gray-400 leading-relaxed">Detailed analysis available in PDF.</td>
                          </tr>
                        );
                      })
                    )}
                  ...
                  {data.risk_areas?.map((r: any, i: number) => (
                    <div key={i} className="flex items-center justify-between p-3 sm:p-4 rounded-md border border-gray-100 dark:border-zinc-800 bg-gray-50/30 dark:bg-zinc-800/40">
                      ...
                      <span className={`text-[9px] sm:text-[10px] font-bold px-2 py-0.5 rounded-full ${
                        r.risk_level === 'High' ? 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400' : 
